...
// ---- İSTEMCİ TARAFI GÖRSEL SIKIŞTIRMA ----
async function compressImage(
  ...
  const blob = await new Promise((resolve) =>
    canvas.toBlob(
      (b) => resolve(b),
      "image/jpeg", // webp de seçilebilir
      quality
    )
  );
...
export default function AdminArticleForm({ initial, onClose, onSaved }) {
  ...
  // edit modundaki mevcut URL (Cloudinary)
  const existingImageUrl = initial?.image?.url || "";

  // title değişirse ve edit değilse slug üret
  useEffect(() => {
  ...
  // seçilen dosya için geçici preview URL'si (lifecycle güvenli)
  ...
  const preview = useMemo(
    ...
    [title, summary, previewUrl, imageAlt, tags]
  );
  ...
  async function onSubmit(e) {
    ...
    try {
      const data = {
        ...
        content,
        imageAlt,
        tags, // backend virgülden bölecek
        status,
      };

      let fileToSend = imageFile || undefined;
      if (imageFile) {
        // upload öncesi sıkıştır
      ...
      if (isEdit) {
        await ArticleAPI.update(initial.slug, data, fileToSend);
      } else {
        await ArticleAPI.create(data, fileToSend);
      }
      onSaved?.();
    } catch (e) {
        ...
        <div className="grid max-h-[calc(90vh-46px)] grid-cols-1 overflow-y-auto md:grid-cols-2">
          ...
          <div className="border-t border-border/60 p-5 md:border-l md:border-t-0">
            ...
            <div className="mt-3 text-[12px] text-muted">
              Kaydedildikten sonra genel sitedeki görünüm değişebilir (ör. uzun
              içerik kısaltma, tipografi).
            </div>
          </div>
        </div>
      </div>
